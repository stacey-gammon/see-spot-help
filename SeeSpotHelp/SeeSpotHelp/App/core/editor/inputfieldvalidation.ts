...
export default class InputFieldValidation {
  ...
  // Makes sure the given input field is one of the possible list items.
  // Updates inputField based on success or failure.
  // @param inputField {InputListField} - the field to validate.
  public static validateListItem(inputField) {
    if (inputField.listItems.indexOf(inputField.value) < 0) {
      inputField.hasError = true;
      inputField.errorMessage = 'Invalid state';
    } else {
      inputField.validated = true;
      inputField.hasError = false;
      inputField.errorMessage = "";
